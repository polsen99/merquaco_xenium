...
def read_transcripts(transcripts_path: Union[str, Path]) -> pd.DataFrame:
    ...
    """
    # Newer xenium experiments
    if transcripts_path.endswith('parquet'):
        try:
            transcripts = pd.read_parquet(transcripts_path)
            return transcripts
        except FileNotFoundError as e:
            raise FileNotFoundError(f'transcripts parquet file not found at {transcripts_path}'
                                    f'error: {e}')
    # MERSCOPE experiments / older xenium experiments
    else:
        try:
            transcripts = pd.read_csv(transcripts_path)
            return transcripts
        except FileNotFoundError as e:
            raise FileNotFoundError(f'transcripts csv not found at {transcripts_path}'
                                    f'error: {e}')
...
def get_fov_neighbors(fovs: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(fovs)):
        # Get target FOV
        fov = fovs.index[i]
        # Find euclidian distance between grid coords of each FOV and all other FOVs
...
class MerscopeExperiment:
    ...
    def __init__(self,
        ...
        # Total transcript counts, including 'Blank' codewordsf
